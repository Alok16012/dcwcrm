...
export function StudentPortalManager() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('students')
      .select(`id, full_name, phone, enrollment_number, status, board_name,
  ...
  // ── Derived filter options (cascade) ──────────────────────────
  const deptOptions     = uniqSorted(students.map(s => (s.department as any)?.name))
  const boardOptions    = uniqSorted(
    students
      .filter(s => !filterDept || (s.department as any)?.name === filterDept)
      .map(s => s.board_name)
  )
  const standardOptions = uniqSorted(
    students
      .filter(s =>
        (!filterDept  || (s.department as any)?.name === filterDept) &&
        (!filterBoard || s.board_name === filterBoard)
      )
      .map(s => (s.sub_course as any)?.name ?? (s.course as any)?.name)
  )
  ...
  const filtered = students.filter(s => {
    ...
    const matchDept  = !filterDept  || (s.department as any)?.name === filterDept
    const matchBoard = !filterBoard || s.board_name === filterBoard
    const matchStd   = !filterStandard ||
      (s.sub_course as any)?.name === filterStandard ||
      (s.course as any)?.name === filterStandard
    return matchSearch && matchDept && matchBoard && matchStd
  })

  // ── Checkbox helpers ──────────────────────────────────────────
  const allChecked = filtered.length > 0 && filtered.every(s => checkedIds.has(s.id))
  const someChecked = filtered.some(s => checkedIds.has(s.id))
  ...
  // ── Bulk Send (all checked students) ─────────────────────────
  async function sendBulkUpdate() {
    if (checkedIds.size === 0) { toast.error('Select at least one student'); return }
    if (!updateTitle.trim())   { toast.error('Title is required'); return }
    if (!updateMessage.trim()) { toast.error('Message is required'); return }
    setBulkSending(true)
    try {
      ...
      const { error } = await supabase.from('student_notifications').insert(rows)
      if (error) throw error
      toast.success(`Update sent to ${checkedIds.size} student${checkedIds.size > 1 ? 's' : ''}!`)
      resetUpdateForm()
  ...
  const selectCls = 'border border-gray-200 rounded-lg px-2 h-8 text-xs bg-white text-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-300 disabled:bg-gray-50 disabled:text-gray-400'

  // ── Render ────────────────────────────────────────────────────
  return (
    <div className="flex flex-col gap-3" style={{ height: 'calc(100vh - 200px)', minHeight: '560px' }}>

      {/* ── Filters + Search bar ─────────────────────────────── */}
      <div className="flex flex-wrap items-center gap-2 bg-white border border-gray-200 rounded-2xl px-3 py-2.5">
        <Filter className="w-4 h-4 text-gray-400 shrink-0" />

        {/* Department */}
        <select className={selectCls} value={filterDept}
          onChange={e => { setFilterDept(e.target.value); setFilterBoard(''); setFilterStandard('') }}>
          <option value="">All Departments</option>
          {deptOptions.map(d => <option key={d} value={d}>{d}</option>)}
        </select>

        {/* Board */}
        <select className={selectCls} value={filterBoard} disabled={!filterDept && boardOptions.length === 0}
          onChange={e => { setFilterBoard(e.target.value); setFilterStandard('') }}>
          <option value="">All Boards</option>
          {boardOptions.map(b => <option key={b} value={b}>{b}</option>)}
        </select>

        {/* Standard / Course */}
        <select className={selectCls} value={filterStandard}
        ...
        <div className="relative flex-1 min-w-40">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 h-8 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-blue-300"
          />
        </div>

        {/* Reset filters */}
        {(filterDept || filterBoard || filterStandard || search) && (
          <button
            onClick={() => { setFilterDept(''); setFilterBoard(''); setFilterStandard(''); setSearch('') }}
        ...
        {/* Count */}
        <span className="text-xs text-gray-400 font-medium shrink-0 ml-auto">
          {filtered.length} student{filtered.length !== 1 ? 's' : ''}
        </span>
      </div>

      {/* ── Bulk send bar (shown when students checked) ───────── */}
      {someChecked && (
        <div className="flex items-center gap-3 bg-blue-50 border border-blue-200 rounded-2xl px-4 py-2.5">
          <Users className="w-4 h-4 text-blue-600 shrink-0" />
          <span className="text-sm font-semibold text-blue-700">
            {checkedIds.size} selected — fill the form below and click Bulk Send
          </span>
          <button
      ...
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        {/* ── Left: Student list ─────────────────────────────────── */}
        <div className={`flex flex-col gap-2 ${selected ? 'hidden md:flex w-72 flex-shrink-0' : 'flex-1'}`}>
          ...
          <div className="flex-1 overflow-y-auto space-y-0.5 pr-1">
            {loading ? (
              ...
              <div key={s.id} className={`flex items-center gap-2 px-2 py-1.5 rounded-xl border transition-all ${
                selected?.id === s.id
                ...
                {/* Serial No */}
                <span className="text-[10px] text-gray-300 font-mono w-5 shrink-0 text-right">{idx + 1}</span>
                {/* Checkbox */}
                <input
                  type="checkbox"
              ...
              {tab === 'updates' && (
                <div className="flex flex-col h-full">
                  ...
                  <div className="p-4 border-b border-gray-100 bg-gray-50/40 space-y-3 shrink-0">
                    ...
                    {/* Category pills with delete on custom */}
                        ...
                        return (
                          <div key={cat} className="relative group flex items-center">
                            <button
                              ...
                              className={`pl-2.5 pr-${isCustom ? '6' : '2.5'} py-1 rounded-full text-xs font-medium border transition-colors ${
                                updateCategory === cat
                            ...
                            {isCustom && (
                              <button
                                onClick={() => {
                                  ...
                                  if (updateCategory === cat) setUpdateCategory(DEFAULT_CATEGORIES[0])
                                }}
                                className="absolute right-1 top-1/2 -translate-y-1/2 w-4 h-4 rounded-full bg-red-100 hover:bg-red-500 text-red-500 hover:text-white flex items-center justify-center transition-colors"
                                title="Delete tab"
                              >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {/* Bulk send (if students selected in left panel) */}
                      {checkedIds.size > 0 && (
                        <Button
                          onClick={sendBulkUpdate}
                          disabled={bulkSending || !updateTitle.trim() || !updateMessage.trim()}
                          variant="outline"
                          className="gap-1.5 border-indigo-300 text-indigo-700 hover:bg-indigo-50 h-9"
                          size="sm"
                        >
                          <Users className="w-3.5 h-3.5" />
                          {bulkSending ? 'Sending…' : `Send to ${checkedIds.size}`}
                        </Button>
                      )}
                ...
                <div className="p-5 space-y-4">
                  ...
                  <div className="bg-white rounded-2xl border border-gray-200 p-4 space-y-3">
                    ...
                    <div className="flex gap-2">
                      <Button className="flex-1 gap-1.5" onClick={saveCredentials} disabled={savingCred || pass.length < 6}>
