...
interface DepartmentRow { id: string; name: string; is_active: boolean; department_sub_sections: SubSectionRow[] }
    ...
    return (
        <div className="border-t border-purple-200 bg-purple-50/40 px-4 py-3 space-y-2">
            <p className="text-xs font-semibold text-purple-700 uppercase tracking-wide mb-2 flex items-center gap-1.5">
                <School className="w-3.5 h-3.5" /> University/Board add karo
                <span className="text-purple-500 font-normal normal-case">(Enter dabao nayi row ke liye)</span>
            </p>
                ...
                {rows.map((row, i) => (
                    <div key={i} className="flex items-center gap-2">
                        ...
                        <Input
                            ...
                            placeholder={`University/Board ${i + 1} ka naam...`}
            ...
            <button
                onClick={addRow}
                className="flex items-center gap-1.5 text-xs text-purple-600 hover:text-purple-800 font-medium mt-1 px-1"
            >
...
export function DepartmentsClient({ departments: initial }: { departments: DepartmentRow[] }) {
    ...
    async function addMultipleSubs(deptId: string, names: string[]) {
        startTransition(async () => {
            ...
            toast.success(`${names.length} items add ho gaye!`)
        })
    }
        ...
        <div className="max-w-2xl">
                            ...
                            {editingDept === dept.id ? (
                                ...
                                <>
                                    ...
                                    <Badge variant="outline" className="text-xs text-gray-500 border-gray-200 mr-1">
                                        {dept.department_sub_sections.length} sub-section{dept.department_sub_sections.length !== 1 ? 's' : ''}
                                    </Badge>
                                    <button
                            ...
                            <div className="border-t border-gray-100">
                                ...
                                {dept.department_sub_sections.length === 0 && addingSubTo !== dept.id && (
                                    <div className="px-16 py-3 text-xs text-gray-400 flex items-center gap-2 bg-gray-50/60">
                                        ...
                                        No sub-sections yet —
                                        <button onClick={() => setAddingSubTo(dept.id)} className="text-purple-600 hover:underline font-medium">
                                            add karo
                                        </button>
                                    </div>
                                )}
                                                ...
                                                {editingSub === sub.id ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            ...
                                                            <button
                                                                onClick={() => setDeleteTarget({ type: 'sub', id: sub.id, name: sub.name, deptId: dept.id })}
            ...
            {deleteTarget && (
                <ConfirmDialog
                    ...
                    confirmLabel="Delete"
                    destructive
                    onConfirm={() => deleteTarget.type === 'dept'
                        ? deleteDept(deleteTarget.id)
                        : deleteSub(deleteTarget.deptId!, deleteTarget.id)
                    }
